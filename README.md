...
| `label-position` | string (default: `top`) | Where to place the label relative to the rectangle; one of `top`, `bottom`, `left`, `right` 
| `label-bg-opacity` | number (default: `0`) | Opacity for the white label background rectangle (range `0` to `1`) 
| `label-auto-boundary` | boolean (default: `true`) | If `true`, tries to keep the label on-canvas by flipping/clamping when it would render offscreen 
